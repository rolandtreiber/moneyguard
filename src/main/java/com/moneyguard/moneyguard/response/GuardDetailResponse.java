...
public class GuardDetailResponse {
    ...
    public GuardDetailResponse(Guard guard) {
        BeanUtils.copyProperties(guard, this);
        setFrequency(guard.getFrequencyType());
        if (guard.getCategories() != null) {
            guard.getCategories().forEach(e -> {
                CategoryDAO categoryDAO = new CategoryDAO();
                BeanUtils.copyProperties(e, categoryDAO);
                categories.add(categoryDAO);
            });
        }
        if (guard.getTags() != null) {
            guard.getTags().forEach(e -> {
        ...
        if (guard.getImportanceLevel() != null) {
            this.setImportanceLevel(guard.getImportanceLevel().getId().toString());
        }
    }
